    ...
    <div class="grid gap-12 lg:grid-cols-[1fr_280px]">
            ...
            <div class="space-y-12">
                @forelse($blogs as $blog)
                    <article class="border-b border-slate-200 pb-10">
                        ...
                        <div class="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs font-semibold uppercase text-slate-500"><span><i class="far fa-user mr-1 text-emerald-600"></i>{{ $blog->author ?: 'Team' }}</span><span><i class="far fa-calendar mr-1 text-emerald-600"></i>{{ $blog->created_at->format('d M Y') }}</span>@if($blog->category)<span class="text-emerald-600">{{ $blog->category }}</span>@endif</div>
        ...
        <aside>
            <div class="space-y-10">
                ...
                <div><h2 class="border-b border-slate-200 pb-3 text-sm font-bold uppercase tracking-wider text-slate-900">Categories</h2><ul class="mt-4 space-y-3 text-sm text-slate-600">@forelse($categories as $category => $total)<li class="flex justify-between border-b border-slate-100 pb-2"><span>{{ $category }}</span><span>({{ $total }})</span></li>@empty<li>No categories yet.</li>@endforelse</ul></div>
                <div><h2 class="border-b border-slate-200 pb-3 text-sm font-bold uppercase tracking-wider text-slate-900">Tag Cloud</h2><div class="mt-4 flex flex-wrap gap-2">@forelse($tags as $tag => $total)<a href="{{ route('blogs', ['search' => $tag]) }}" class="bg-emerald-600 px-3 py-1.5 text-[10px] font-bold uppercase text-white hover:bg-emerald-700">{{ $tag }}</a>@empty<span class="text-sm text-slate-500">No tags yet.</span>@endforelse</div></div>
                <div><h2 class="border-b border-slate-200 pb-3 text-sm font-bold uppercase tracking-wider text-slate-900">Archives</h2><ul class="mt-4 space-y-3 text-sm text-slate-600">@forelse($archives as $archive)<li class="flex justify-between border-b border-slate-100 pb-2"><span>{{ \Carbon\Carbon::createFromFormat('Y-m', $archive->month)->format('F Y') }}</span><span>({{ $archive->total }})</span></li>@empty<li>No archives yet.</li>@endforelse</ul></div>
            </div>
        </aside>
    </div>
</main>
