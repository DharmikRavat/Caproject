                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="parent_service_id" class="form-control">
                            ...
                            @foreach($parentServices as $s)
                                <option value="{{ $s->id }}" {{ old('parent_service_id', $service->parent_service_id) == $s->id ? 'selected' : '' }}>{{ $s->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-12 mb-3">
                        ...
                        @if($service->header_image)
                            <div class="mt-2">
                                <img src="{{ \Illuminate\Support\Str::startsWith($service->header_image, ['http://', 'https://']) ? $service->header_image : Storage::url($service->header_image) }}" alt="Current Header Image" class="img-thumbnail" style="max-height: 100px;">
                            </div>
                        @endif
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="form-label">FontAwesome Icon Class</label>
                        <input type="text" name="icon" class="form-control" value="{{ old('icon', $service->icon) }}" placeholder="e.g. fas fa-file-signature">
                    </div>
                    <div class="col-md-6 mb-3">
                        ...
                        @if($service->featured_image)
                            <div class="mt-2">
                                <img src="{{ \Illuminate\Support\Str::startsWith($service->featured_image, ['http://', 'https://']) ? $service->featured_image : Storage::url($service->featured_image) }}" alt="Current Image" class="img-thumbnail" style="max-height: 80px;">
                            </div>
                        @endif
                    </div>
                    <div class="col-md-12 mb-3">
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea name="description" class="form-control summernote" rows="5">{{ old('description', $service->description) }}</textarea>
                    </div>
                    <div class="col-md-6 mb-3">
